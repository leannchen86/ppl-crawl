...
Analyzes the first few images in ppl-images/aaron to detect faces.
"""
...
print(f"\nImages with faces detected ({len(faces_found)}/{images_count}):")
for img, status, count in faces_found:
    print(f"   ✓ {img} ({count} face{'s' if count > 1 else ''})")

if no_faces:
    print(f"\nImages with NO faces detected ({len(no_faces)}/{images_count}):")
    for img, status, count in no_faces:
        print(f"   ✗ {img}")

if errors:
    print(f"\nImages not found ({len(errors)}/{images_count}):")
