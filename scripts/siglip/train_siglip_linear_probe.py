"""
SigLIP Linear Probe Training for Face-Name Classification.

This script trains a linear classifier on top of frozen SigLIP embeddings
...
2. SigLIP-base-patch16-224 - modern CLIP variant with better calibration (Google, 2023)
...
Usage:
    ...
    python scripts/siglip/train_siglip_linear_probe.py \\
        ...
        --output-dir ./results/siglip/balanced

    # Train with imbalanced data
    python scripts/siglip/train_siglip_linear_probe.py \\
        --data-dir /home/leann/face-detection/data/siglip_imbalanced \\
        --output-dir ./results/siglip/imbalanced

Model info:
- SigLIP-base-patch16-224: ~200M params, embedding dim 768
- Trained with sigmoid loss (not softmax) for better calibration
...
# SigLIP can be loaded via open_clip or transformers
# We use open_clip for consistency with existing CLIP code
try:
    import open_clip
    USING_OPEN_CLIP = True
except ImportError:
...
def load_siglip_model(model_name: str = "ViT-B-16-SigLIP", pretrained: str = "webli", device: str = "cuda"):
    """Load SigLIP model and preprocessing transforms."""

    if USING_OPEN_CLIP:
        print(f"Loading SigLIP via open_clip: {model_name} (pretrained={pretrained})")
        ...
        print(f"Loading SigLIP via transformers: google/siglip-base-patch16-224")
...
def extract_embeddings(
    model, dataloader, device: str, use_open_clip: bool = True
...
def main():
    parser = argparse.ArgumentParser(description="SigLIP Linear Probe Training")
    ...
    parser.add_argument("--model-name", type=str, default="ViT-B-16-SigLIP",
                        help="SigLIP model variant (default: ViT-B-16-SigLIP)")
    parser.add_argument("--pretrained", type=str, default="webli",
                        help="Pretrained weights (default: webli)")
    ...
    print("=" * 70)
    print("SIGLIP LINEAR PROBE TRAINING")
    print("=" * 70)
    ...
    model, preprocess, embed_dim = load_siglip_model(
    ...
    start_time = time.time()
    train_X, train_y = extract_embeddings(model, train_loader, device, use_open_clip=USING_OPEN_CLIP)
    val_X, val_y = extract_embeddings(model, val_loader, device, use_open_clip=USING_OPEN_CLIP)
    ...
    # Print summary
    print("\n" + "=" * 70)
    print("SIGLIP LINEAR PROBE RESULTS")
    print("=" * 70)
