"""
Unified Linear Probe Training for Face-Name Classification.

This script trains a linear classifier on top of frozen vision model embeddings
for the "Guessing Your English Name" project. Supports CLIP, SigLIP, and OpenCLIP
in a single unified framework for fair comparison.

Key design decisions:
1. Linear probe (frozen backbone) - proven to outperform fine-tuning for this task
2. Model registry pattern - supports CLIP, SigLIP, OpenCLIP with identical training
3. Standardized hyperparameters - AdamW, lr=0.01, weight_decay=1e-4, epochs=100
4. Uses pre-created held-out validation split for fair comparison

Model configurations:
- CLIP: ViT-B-32 (openai), embedding dim 512
- SigLIP: ViT-B-16-SigLIP (webli), embedding dim 768
- OpenCLIP: ViT-B-32 (laion2b_s34b_b79k), embedding dim 512

Usage:
    # Train CLIP
    python scripts/unified/train_linear_probe.py \\
        --model clip \\
        --data-dir data/siglip_balanced \\
        --output-dir results/linear_probe_comparison/clip/balanced

    # Train SigLIP
    python scripts/unified/train_linear_probe.py \\
        --model siglip \\
        --data-dir data/siglip_balanced \\
        --output-dir results/linear_probe_comparison/siglip/balanced

    # Train OpenCLIP
    python scripts/unified/train_linear_probe.py \\
        --model openclip \\
        --data-dir data/siglip_balanced \\
        --output-dir results/linear_probe_comparison/openclip/balanced
"""
import argparse
import json
import os
...
def load_model(model_key: str, device: str = "cuda"):
    ...
    Args:
        model_key: One of "clip", "siglip", "openclip"
    ...
    Returns:
        ...
        model_config: Configuration dict from registry
    """
    ...
    print(f"Loading {config['type']}: {model_name} (pretrained={pretrained})")
    ...
    if embed_dim != config["expected_embed_dim"]:
        print(f"  Warning: Expected embed_dim={config['expected_embed_dim']}, got {embed_dim}")

    return model, preprocess, embed_dim, config
...
def train_linear_probe(
    ...
    device: str = "cuda",
    verbose: bool = True,
) -> Tuple[float, int, Dict[int, float], nn.Module, List[Dict]]:
    """
    Train a linear classifier on extracted embeddings.

    Returns:
        best_val_acc: Best validation accuracy achieved
        best_epoch: Epoch when best accuracy was achieved
        per_class_acc: Per-class accuracy at best epoch
        classifier: Trained classifier (with best weights loaded)
        history: Training history (epoch-by-epoch metrics)
    """
    ...
    return best_val_acc, best_epoch, best_per_class, classifier, history
...
def main():
    parser = argparse.ArgumentParser(
        description="Unified Linear Probe Training for CLIP/SigLIP/OpenCLIP",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python scripts/unified/train_linear_probe.py --model clip --data-dir data/siglip_balanced --output-dir results/clip/balanced
  python scripts/unified/train_linear_probe.py --model siglip --data-dir data/siglip_imbalanced --output-dir results/siglip/imbalanced
        """
    )
    ...
    print("=" * 70)
    print(f"UNIFIED LINEAR PROBE TRAINING - {model_config['type'].upper()}")
    print("=" * 70)
    print(f"Model: {model_config['model_name']} (pretrained={model_config['pretrained']})")
    ...
    print(f"Holdout checksum: {holdout_checksum}")
    ...
    model, preprocess, embed_dim, _ = load_model(args.model, device)
    ...
    best_acc, best_epoch, per_class_acc, classifier, history = train_linear_probe(
    ...
    # Print summary
    print("\n" + "=" * 70)
    print(f"{model_config['type'].upper()} LINEAR PROBE RESULTS")
    print("=" * 70)
    ...
    # Convert per_class_acc to use name keys instead of indices
    per_class_acc_named = {names[idx]: acc for idx, acc in per_class_acc.items()}
    ...
    experiment_config = {
        ...
        "per_class_accuracy": per_class_acc_named,
        "paths": {
